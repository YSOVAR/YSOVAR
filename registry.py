...
:class:`atlas.YSOVAR_atlas` objects can autogenerate a certain number of colums
in the data table. This module provides the mechanism for defining the
...
that :class:`atlas.YSOVAR_atlas` can find them and call them with the the right
parameters.
...
This is probably best explained by an example::
    ...
    registry.register(np.mean, n_bands=1, error = False, time = False, name = "mean", default_colnames = ["mean"], description = "mean of lightcurve")
...
.. note::
    In practice, you do not need to register ``np.mean`` as a selection of
